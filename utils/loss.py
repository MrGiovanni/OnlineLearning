...
class DiceLoss_SM(nn.Module):
    ...
    def forward(self, predict, target, name, ratio, TEMPLATE):
        ...
        for b in range(B):
            ...
            for organ in organ_list:
                ...
                batch_loss.append(dice_loss*ratio[b][organ-1])
            
        total_loss.append(torch.stack(batch_loss).mean())

        return torch.stack(total_loss)

  
class Multi_BCELoss_SM(nn.Module):
    def __init__(self, ignore_index=None, num_classes=3, **kwargs):
        super(Multi_BCELoss_SM, self).__init__()
        self.kwargs = kwargs
        self.num_classes = num_classes
        self.ignore_index = ignore_index
    ...
    def forward(self, predict, target, name, ratio, TEMPLATE):
        ...
        for b in range(B):
            ...
            for organ in organ_list:
                ...
                batch_loss.append(ce_loss*ratio[b][organ-1])
            total_loss.append(torch.stack(batch_loss).mean())
